...
The theoretical mean and standard deviation for the exponential distribution used are presented below:
```{r}
theo_mean <- 1/rate
theo_sd <- 1/rate
```
Theoretical Mean: `r theo_mean` ; Theoretical Standard Deviation: `r theo_sd`
...
for(i in 1:nosim){
  ...
  df[i,1] <- mean(x)
  df[i,2] <- sd(x)
  df[i,3] <- ((mean(x)-theo_mean)/(theo_sd/sqrt(sample_size)))
}
```

Let us see how does the sample means and standard deviations compare against the theoretical mean and standard deviation of the population.
...
As expected the distribution of sample means and standard deviations is centered around its theroretical poulation mean and standard deviation respectively.
